...
public class ContainmentBinder {
    ...
    public void handleContainmentLogicalOperator(
            ContainmentLogicalOperator dto, MutablePair<ContainmentExpression, Map<Integer, Containment>> result) {
        ContainmentExpression containmentExpression;
        Map<Integer, Containment> containmentMap;
        Pair<ContainmentExpression, Map<Integer, Containment>> pair = buildLogicalOperator(
                dto.getSymbol(),
        ...
        for (int i = 2; i < dto.getValues().size(); i++) {
            Pair<ContainmentExpression, Map<Integer, Containment>> subPair = buildLogicalOperator(
                    dto.getSymbol(), pair, buildContainment(dto.getValues().get(i)));

...
public class AqlBinder {
    ...
    public Pair<EntityQuery<Record>, List<ParameterValue>> bind(AqlDto aqlDto) {
        ...
        Map<Containment, String> variablesMap = buildVariableMap(aqlDto.getContains()).entrySet().stream()
        ...
        // build select
        SelectAqlField<?>[] selectAqlFields = aqlDto.getSelect().getStatement().stream()
        ...
        if (aqlDto.getWhere() != null) {
            Pair<Condition, List<ParameterValue>> conditionPair = whereBinder.bind(aqlDto.getWhere(), containmentMap);

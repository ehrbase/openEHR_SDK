...
import static org.ehrbase.openehr.sdk.serialisation.flatencoding.std.umarshal.StdToCompositionWalker.findRMUnmarshaller;
import static org.ehrbase.openehr.sdk.serialisation.walker.FlatHelper.buildDummyChild;
import static org.ehrbase.openehr.sdk.serialisation.walker.FlatHelper.isExactlyDvCodedText;
    ...
    public void process(
        ...
        FlatHelper.extractMultiValued(term, "_other_reference_ranges", values).forEach((k, v) -> {
            ...
            Map<FlatPathDto, String> meaningValues =
                    FlatHelper.filter(values, term + "/_other_reference_ranges:" + k + "/meaning", false);

            if (!meaningValues.isEmpty()) {
                final DvText meaning;
                String meaningAttributeName = "meaning";
                boolean isDvCodedText =
                        isExactlyDvCodedText(meaningValues, term + "/_other_reference_ranges:" + k + "/meaning");
                if (isDvCodedText) {
            ...
            handleNormalRange(
                    v, consumedPaths, context, term + "/_other_reference_ranges:" + k, referenceRange::setRange);
        });
        ...
        if (!rangeValues.isEmpty()) {
            ...
            callPostProcess(
                    ...
                    context.getNodeDeque()
                            .peek()
                            .findChildById("range")
                            .orElse(FlatHelper.buildDummyChild(
                                    "range", context.getNodeDeque().peek())));
        }
    }

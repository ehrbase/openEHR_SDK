...
import static org.ehrbase.openehr.sdk.serialisation.walker.FlatHelper.findOrBuildSubNode;
        ...
        callUnmarshal(
                ...
                context.getNodeDeque()
                        ...
                        .orElse(buildDummyChild(
                                attributeId, context.getNodeDeque().peek())));
    ...
    protected void callPostProcess(
        ...
        if (!subValues.isEmpty()) {
            StdToCompositionWalker.findUnmarshalPostprocessors(rmObject.getClass())
                    .forEach(p ->
                            ((UnmarshalPostprocessor) p).process(newTerm, rmObject, subValues, consumedPaths, context));
        }
        ...
        if (!values.isEmpty()) {
            ...
            if (FlatHelper.isExactlyPartyRelated(values, currentTerm + "/" + id, null)) {
                ...
                throw new UnmarshalException(
                        String.format("Could not find concrete instance of Party proxy for %s/%s", currentTerm, id));
            }
            ...
            callPostProcess(
                    currentTerm, id, partyProxy, values, consumedPaths, context, findOrBuildSubNode(context, id));
        }
    }
}

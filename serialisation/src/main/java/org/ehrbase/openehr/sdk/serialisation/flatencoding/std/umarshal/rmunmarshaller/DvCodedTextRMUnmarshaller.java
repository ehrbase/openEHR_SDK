...
public class DvCodedTextRMUnmarshaller extends AbstractRMUnmarshaller<DvCodedText> {
    ...
    public void handle(
        ...
        if (rmObject.getDefiningCode() == null
                || rmObject.getDefiningCode().getTerminologyId() == null
                || rmObject.getDefiningCode().getTerminologyId().getValue() == null) {
        ...
        if (rmObject.getDefiningCode() != null && rmObject.getDefiningCode().getCodeString() != null) {
            currentValues.keySet().stream()
                    ...
                    .filter(k -> StringUtils.substringAfter(k, "|")
    ...
    private void setFromNode(
            DvCodedText rmObject, Context<Map<FlatPathDto, String>> context, Predicate<WebTemplateInputValue> filter) {
        Optional.of(context.getNodeDeque().peek().getInputs()).stream()
                .flatMap(List::stream)
                .filter(i -> "code".equals(i.getSuffix()))

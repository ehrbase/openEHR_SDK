...
/**
 * decorator for DvTime. Add attributes and handling for partial time (f.e. 10:00) Provide a
 * defaulted representation (f.e. 10:00:00 for the above) Calculate a timestamp based on the
...
public class DvTimeAttributes extends TemporalAttributes implements I_TimeAttributes {
  ...
  public static DvTimeAttributes instanceFromValue(DvTime dvTime) {
    ...
    } else throw new IllegalArgumentException("Invalid time:" + dvTime.getValue().toString());

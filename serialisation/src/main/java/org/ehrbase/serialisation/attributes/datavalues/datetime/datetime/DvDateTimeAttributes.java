...
/**
 * decorator for DvDateTime. Add attributes and handling for partial date/time (f.e. 2019-12)
 * Provide a defaulted representation (f.e. 2019-12-01:00:00:00Z for the above) Calculate a
 * timestamp based on the defaulted representation
 */
public class DvDateTimeAttributes extends TemporalAttributes implements I_DateTimeAttributes {
  ...
  private DvDateTimeAttributes(
      DvDateTime dvDateTime, I_DateAttributes datePart, I_TimeAttributes timePart) {
  ...
  public static DvDateTimeAttributes instanceFromValue(DvDateTime dvDateTime) {
    ...
    DvDateAttributes dvDateAttributes =
        DvDateAttributes.instanceFromValue(new DvDate((Temporal) localDate));
  ...
  public Integer getSupportedChronoFields() {
    return TemporalAttributes.DV_DATE_TIME
        ...
        | (timePart == null
            ? 0
            : timePart.getSupportedChronoFields() & ~TemporalAttributes.DV_TIME);
  }

...
public class LightRawJsonEncoder {
  ...
  public String encodeContentAsString(String root) {
    ...
    GsonBuilder gsonRaw =
        EncodeUtilArchie.getGsonBuilderInstance(I_DvTypeAdapter.AdapterType.DBJSON2RAWJSON);
    String raw = null;
    if (fromDB instanceof Map) {
      if (root != null) {
        Object contentMap = ((Map) fromDB).get(root);
        if (contentMap instanceof LinkedTreeMap
            && ((LinkedTreeMap) contentMap).size() == 0) // empty content
  ...
  public JsonElement encodeContentAsJson(String root) {
    ...
    JsonElement jsonElement =
        gsonRaw.create().toJsonTree(db2map(root != null && root.equals("value")));
    if (root != null) {
      // in order to create the canonical form, build the ELEMENT json (hence the type is passed
      // into the embedded value)

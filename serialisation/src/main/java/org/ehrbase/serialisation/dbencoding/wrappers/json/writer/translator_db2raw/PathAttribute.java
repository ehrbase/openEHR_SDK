...
public class PathAttribute {
  ...
  String nodeNameFromPath(String key) {
    ...
    if (startAt >= 0) {
      return path.substring(
          path.indexOf(namePart, startAt) + namePart.length(), path.indexOf("]", startAt) - 1);
    }
    return null;
  }
  ...
  String parentArchetypeNodeId(String key) {
    ...
    if (key.startsWith("/")) compareKey = compareKey.substring(1);
    if (compareKey.endsWith("]")) compareKey = compareKey.substring(0, compareKey.length() - 1);
      ...
      if (pathNode.startsWith(compareKey)) {
        if (i > 0) {
          ...
          if (parentNode.contains(AND_NAME_VALUE))
            parentArchetypeNodeId =
                parentNode
                    .substring(parentNode.indexOf("[") + 1, parentNode.indexOf(AND_NAME_VALUE))
                    .trim();
          else
            parentArchetypeNodeId =
                parentNode.substring(parentNode.indexOf("[") + 1, parentNode.indexOf("]")).trim();
        ...
        if (i > 0) {
          ...
          if (parentNode.contains(AND_NAME_VALUE))
            parentArchetypeNodeId =
                parentNode
                    .substring(
                        parentNode.indexOf(AND_NAME_VALUE) + AND_NAME_VALUE.length(),
  ...
  String findPath(Object value) {
    ...
    } else return usePath(foundPath, (LinkedTreeMap) value);
    return null;
  }
  ...
  String archetypeNodeId() {
    ...
    if (lastSegment.contains(AND_NAME_VALUE))
      archetypeNodeId =
          lastSegment
              .substring(lastSegment.indexOf("[") + 1, lastSegment.indexOf(AND_NAME_VALUE))
              .trim();
    else
      archetypeNodeId =
          lastSegment.substring(lastSegment.indexOf("[") + 1, lastSegment.indexOf("]")).trim();

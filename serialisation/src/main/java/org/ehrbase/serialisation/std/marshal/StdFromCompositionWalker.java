...
import org.ehrbase.serialisation.std.marshal.config.DefaultStdConfig;
import org.ehrbase.serialisation.std.marshal.config.StdConfig;
import org.ehrbase.serialisation.std.marshal.postprocessor.MarshalPostprocessor;
...
public class StdFromCompositionWalker extends FromCompositionWalker<Map<String, Object>> {
    ...
    protected void preHandle(Context<Map<String, Object>> context) {
        ...
        if (!visitChildren(context.getNodeDeque().peek())) {
            ...
            context.getObjectDeque().peek().putAll(stdConfig.buildChildValues(buildNamePath(context), currentObject, context));
        }
    ...
    protected void postHandle(Context<Map<String, Object>> context) {
        List<MarshalPostprocessor> postprocessor = Stream.concat(Stream.of(context.getRmObjectDeque().peek().getClass()), ClassUtils.getAllSuperclasses(context.getRmObjectDeque().peek().getClass()).stream())

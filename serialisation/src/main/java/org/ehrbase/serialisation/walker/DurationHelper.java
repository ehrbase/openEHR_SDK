...
public class DurationHelper {
   ...
   * @return
   */
  public static Optional<WebTemplateComparisonSymbol> getTotalComparisonSymbol(
      WebTemplateNode node, MIN_MAX minMax) {

    List<WebTemplateComparisonSymbol> symbols =
        Optional.ofNullable(node).map(WebTemplateNode::getInputs).stream()
            .flatMap(List::stream)
            .map(WebTemplateInput::getValidation)
    ...
    return symbols.stream()
        .filter(
            s ->
                List.of(WebTemplateComparisonSymbol.GT, WebTemplateComparisonSymbol.LT).contains(s))
        .findAny()

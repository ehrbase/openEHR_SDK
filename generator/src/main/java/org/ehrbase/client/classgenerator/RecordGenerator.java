...
public class RecordGenerator {
    ...
    private void buildQuery(HashMap<Integer, TypeSpec> typeSpecHashMap) throws IOException {
        ...
        builder.addMethod(MethodSpec.methodBuilder("buildEntityQuery")
                ...
                .addParameter(ArrayTypeName.of(ParameterizedTypeName.get(ClassName.get(SelectAqlField.class), WildcardTypeName.subtypeOf(Object.class))), "selectFields")
                .addCode("return new EntityQuery<>(containment, selectFields);")
                .varargs(true)
                .returns(ParameterizedTypeName.get(EntityQuery.class, Record.class)
                ).build());

        builder.addMethod(MethodSpec.methodBuilder("buildNativeQuery")
                .addModifiers(Modifier.STATIC, Modifier.PUBLIC)
                .addParameter(String.class, "aql")
                .addParameter(ArrayTypeName.of(ParameterizedTypeName.get(ClassName.get(Class.class), WildcardTypeName.subtypeOf(Object.class))), "expected")
                .addCode(CodeBlock.builder().addStatement("return new NativeQuery<>(aql, $T.stream(expected).map(AqlField::create).toArray($T<?>[]::new))", ClassName.get(Arrays.class), ClassName.get(AqlField.class)).build())
                .varargs(true)
    ...
    private MethodSpec buildEntityQueryMethod(TypeSpec recordNTypeSpec) {
        ...
        methodBuilder.returns(ParameterizedTypeName.get(ClassName.get(EntityQuery.class), ParameterizedTypeName.get(ClassName.get(RECORD_PACKAGE_NAME, recordNTypeSpec.name), recordNTypeSpec.typeVariables.toArray(TypeName[]::new))));
        methodBuilder.addParameter(ContainmentExpression.class, "containment");
        recordNTypeSpec.typeVariables.forEach(t -> methodBuilder.addParameter(ParameterizedTypeName.get(ClassName.get(SelectAqlField.class), t), "selectField" + t.name.replace("T", "")));
    ...
    private MethodSpec buildNativeQueryMethod(TypeSpec recordNTypeSpec) {
        ...
        methodBuilder.addModifiers(Modifier.PUBLIC, Modifier.STATIC);
        recordNTypeSpec.typeVariables.forEach(methodBuilder::addTypeVariable);
        methodBuilder.returns(ParameterizedTypeName.get(ClassName.get(NativeQuery.class), ParameterizedTypeName.get(ClassName.get(RECORD_PACKAGE_NAME, recordNTypeSpec.name), recordNTypeSpec.typeVariables.toArray(TypeName[]::new))));
        methodBuilder.addParameter(String.class, "aql");
        recordNTypeSpec.typeVariables.forEach(t -> methodBuilder.addParameter(ParameterizedTypeName.get(ClassName.get(Class.class), t), "expected" + t.name.replace("T", "")));
    ...
    private void buildAbstractRecordImp(HashMap<Integer, TypeSpec> typeSpecHashMap) throws IOException {
        ...
        for (int n = 1; n <= MAX_RECORD_SIZE; n++) {
            ...
            TypeSpec recordNTypeSpec = typeSpecHashMap.get(n);
            builder.addSuperinterface(ParameterizedTypeName.get(ClassName.get(RECORD_PACKAGE_NAME, recordNTypeSpec.name), recordNTypeSpec.typeVariables.toArray(TypeName[]::new)));


            builder.addMethod(MethodSpec.methodBuilder("value" + n).addModifiers(Modifier.PUBLIC).addCode(CodeBlock.builder().addStatement("return ($T) value(" + (n - 1) + ")", typeVariable).build()).returns(typeVariable).build());
            builder.addMethod(MethodSpec.methodBuilder("field" + n).addModifiers(Modifier.PUBLIC).addCode(CodeBlock.builder().addStatement("return ($T) field(" + (n - 1) + ")", ParameterizedTypeName.get(ClassName.get(AqlField.class), typeVariable)).build()).returns(ParameterizedTypeName.get(ClassName.get(AqlField.class), typeVariable)).build());
    ...
    private TypeSpec buildRecordN(int n) throws IOException {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            builder.addTypeVariable(typeVariable);
            builder.addMethod(MethodSpec.methodBuilder("value" + i).addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC).returns(typeVariable).build());
            builder.addMethod(MethodSpec.methodBuilder("field" + i).addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC).returns(ParameterizedTypeName.get(ClassName.get(AqlField.class), typeVariable)).build());
        }

...
public class RecordGenerator {
    ...
    private void buildQuery(HashMap<Integer, TypeSpec> typeSpecHashMap) throws IOException {
        ...
        builder.addMethod(MethodSpec.methodBuilder("buildAql")
                .addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC)
                .returns(String.class)
                .build());
        builder.addMethod(MethodSpec.methodBuilder("fields")
                .addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC)
                .returns(ArrayTypeName.of(ParameterizedTypeName.get(ClassName.get(AqlField.class), TypeName.OBJECT)))
                .build());
        ...
        builder.addMethod(MethodSpec.methodBuilder("buildEntityQuery")
                ...
                .addParameter(
                        ArrayTypeName.of(ParameterizedTypeName.get(
                                ClassName.get(SelectAqlField.class), WildcardTypeName.subtypeOf(Object.class))),
                        "selectFields")
    ...
    private MethodSpec buildEntityQueryMethod(TypeSpec recordNTypeSpec) {
        ...
        recordNTypeSpec.typeVariables.forEach(t -> methodBuilder.addParameter(
                ParameterizedTypeName.get(ClassName.get(SelectAqlField.class), t),
                "selectField" + t.name.replace("T", "")));

        String vars = recordNTypeSpec.typeVariables.stream()
    ...
    private MethodSpec buildNativeQueryMethod(TypeSpec recordNTypeSpec) {
        ...
        recordNTypeSpec.typeVariables.forEach(t -> methodBuilder.addParameter(
                ParameterizedTypeName.get(ClassName.get(Class.class), t), "expected" + t.name.replace("T", "")));
    ...
    private void buildAbstractRecordImp(HashMap<Integer, TypeSpec> typeSpecHashMap) throws IOException {
        ...
        for (int n = 1; n <= MAX_RECORD_SIZE; n++) {
            ...
            builder.addMethod(MethodSpec.methodBuilder("value" + n)
                    ...
                    .build());
            builder.addMethod(MethodSpec.methodBuilder("field" + n)
                    .addModifiers(Modifier.PUBLIC)
                    .addCode(CodeBlock.builder()
                            .addStatement(
                                    "return ($T) field(" + (n - 1) + ")",
    ...
    private TypeSpec buildRecordN(int n) throws IOException {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            builder.addTypeVariable(typeVariable);
            builder.addMethod(MethodSpec.methodBuilder("value" + i)
                    .addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC)
                    .returns(typeVariable)
                    .build());
            builder.addMethod(MethodSpec.methodBuilder("field" + i)
                    .addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC)
                    .returns(ParameterizedTypeName.get(ClassName.get(AqlField.class), typeVariable))
                    .build());
        }

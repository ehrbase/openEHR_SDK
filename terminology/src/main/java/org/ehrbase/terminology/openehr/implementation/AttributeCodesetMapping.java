...
/**
 * This class provide mappings between RM object attributes and their corresponding entry into
 * openehr terminology F.e. mapping between Composition.lifecycle_state and its matching entry:
 * 'version lifecycle state' in openehr terminology (en) or 'estado de ciclo de vida de versão' (pt)
 */
public class AttributeCodesetMapping {
  ...
  public static AttributeCodesetMapping getInstance(String xmlfilename)
      ...
      for (int idx = 0; idx < mapList.getLength(); idx++) {
        ...
        if (element.getElementsByTagName("terminology") != null
          ...
          throw new IllegalArgumentException(
              "no terminology specified for entry:" + element.toString());
        }
      }
    } catch (Exception e) {

 ...
 */
package org.ehrbase.openehr.sdk.client.openehrclient.defaultrestclient.systematic.ehrquery;
...
import org.ehrbase.openehr.sdk.client.openehrclient.defaultrestclient.systematic.comparator.EhrComparator;
...
public class CanonicalEhrQuery1IT extends CanonicalUtil {
    ...
    public void testEhrAttributes() {
        ...
        Query<Record1<Map>> query =
                Query.buildNativeQuery("select e " + "from EHR e[ehr_id/value = $ehr_id]", Map.class);
        ...
        Assertions.assertThat(new EhrComparator(referenceEhrStatus, ehrUUID, actualDvDateTime).compare(valueMap))
                .isNull();
    }
    ...
    public void testEhrAttributesDrillDown1() {
        ...
        Query<Record1<Map>> query =
                Query.buildNativeQuery("select e/ehr_status/name " + "from EHR e[ehr_id/value = $ehr_id]", Map.class);

        List<Record1<Map>> result = openEhrClient.aqlEndpoint().execute(query, new ParameterValue("ehr_id", ehrUUID));

        Map<String, Object> valueMap = result.get(0).value1();

        assertThat(compareArbitraryRmClass(valueMap, DvText.class, referenceEhrStatus.getName()))
                .isNull();
    }
    ...
    public void testEhrAttributesDrillDown11() {
        ...
        Query<Record1<Map>> query = Query.buildNativeQuery(
                "select e/ehr_status/archetype_node_id " + "from EHR e[ehr_id/value = $ehr_id]", Map.class);
    ...
    public void testEhrAttributesDrillDown2() {
        Query<Record1<Map>> query = Query.buildNativeQuery(
                "select e/ehr_status/name/value " + "from EHR e[ehr_id/value = $ehr_id]", Map.class);

        List<Record1<Map>> result = openEhrClient.aqlEndpoint().execute(query, new ParameterValue("ehr_id", ehrUUID));

        assertThat(result.get(0).value(0))
                .as("e/ehr_status/name/value")
                .isEqualTo(attributeValueAt(referenceEhrStatus.getName(), "value"));
    }

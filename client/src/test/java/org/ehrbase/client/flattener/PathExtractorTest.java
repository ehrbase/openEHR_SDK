...
public class PathExtractorTest {
    ...
    public void testPathExtractorWithName() {
        PathExtractor cut = new PathExtractor("/content[openEHR-EHR-SECTION.adhoc.v1 and name/value='Allgemeine Angaben']");
        assertThat(cut.getAttributeName()).isNull();
        assertThat(cut.getChildName()).isEqualTo("content");
        assertThat(cut.getChildPath()).isEqualTo("/content[openEHR-EHR-SECTION.adhoc.v1 and name/value='Allgemeine Angaben']");
        assertThat(cut.getParentPath()).isEqualTo("/");
    }
    ...
    public void testPathExtractorWithAttributeAndAtCode() {
        PathExtractor cut = new PathExtractor("/data[at0001]/events[at0002]/data[at0003]/items[at0004]|magnitude");
        assertThat(cut.getAttributeName()).isEqualTo("magnitude");
        assertThat(cut.getChildName()).isEqualTo("items");
        assertThat(cut.getChildPath()).isEqualTo("/data[at0001]/events[at0002]/data[at0003]/items[at0004]");

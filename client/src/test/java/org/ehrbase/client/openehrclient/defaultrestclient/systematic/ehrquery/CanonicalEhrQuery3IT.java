...
public class CanonicalEhrQuery3IT extends CanonicalCompoAllTypeQueryIT {
    ...
    public void setUp(){
        try {
            referenceEhrStatus = new CanonicalJson().unmarshal(IOUtils.toString(EhrTestDataCanonicalJson.EHR_STATUS_SUBJECT_EXTERNAL_REF_OTHER_DETAILS.getStream(), StandardCharsets.UTF_8), EhrStatus.class);
        } catch (IOException e) {
            e.printStackTrace();
        }

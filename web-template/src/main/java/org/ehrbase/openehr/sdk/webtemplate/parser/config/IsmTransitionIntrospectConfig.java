...
public class IsmTransitionIntrospectConfig implements RmIntrospectConfig {
    ...
    public ValueSet findExternalValueSet(String fieldName) {
        switch (fieldName) {
            case "currentState":
                return TerminologyProvider.findOpenEhrValueSet(OPENEHR, "instruction states", "en");
            case "transition":
                return TerminologyProvider.findOpenEhrValueSet(OPENEHR, "instruction transitions", "en");
            default:

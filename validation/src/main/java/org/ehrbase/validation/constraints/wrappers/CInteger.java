...
public class CInteger extends CConstraint implements I_CTypeValidate {
  ...
  public void validate(String path, Object aValue, CPRIMITIVE cprimitive)
    ...
    if (aValue instanceof Integer) integer = (Integer) aValue;
    else if (aValue instanceof Long) integer = ((Long) aValue).intValue();
    else
      ValidationException.raise(
          path,
          "Value is not a supported type for Integer:" + aValue.getClass().getSimpleName(),
          "INT02");
    ...
    if (cinteger.sizeOfListArray() > 0 && !ArrayUtils.contains(cinteger.getListArray(), integer))
      ValidationException.raise(
          path, "Integer value does not match any values in constraint:" + integer, "INT01");
  }
}

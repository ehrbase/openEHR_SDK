...
public class OptConstraint {
   ...
   * @throws IllegalArgumentException
   */
  private void handleArchetypeRoot(
      OPERATIONALTEMPLATE opt, CARCHETYPEROOT def, String name, String path)
      throws IllegalArgumentException {
      ...
      if (ccobj.getAttributesArray().length > 0) {
        ...
        if (obj instanceof CSINGLEATTRIBUTE) {
          ...
          if (VALUE.equals(attr.getRmAttributeName())) {
            ...
            } else log.debug("ELEMENT without child, assuming ANY type");
          } else {
            log.debug(
                "additional attribute found for element attr.getRmAttributeName()="
  ...
  private void handleDvOrdinal(CDVORDINAL cdo, Map<String, String> termDef, String path)
      throws IllegalArgumentException {
  ...
  private void handleDomainTypeObject(CDOMAINTYPE cpo, Map<String, String> termDef, String path)
      throws IllegalArgumentException {

    if (cpo instanceof CDVORDINAL) {
      if (((CDVORDINAL) cpo).isSetAssumedValue()) {
        // do nothing
      } else handleDvOrdinal((CDVORDINAL) cpo, termDef, path);
    }
  }
}

  ...
  public void validate(String path, Object aValue, CPRIMITIVE cprimitive)
    ...
    if (!(aValue instanceof Temporal))
      ValidationException.raise(
          path, "INTERNAL: Time validation expects a java-date-time argument", "DATE_01");

    CDATE cdate = (CDATE) cprimitive;
    String dvDateStr = aValue.toString();

    // check pattern if any
    new DateTimeSyntax(path, dvDateStr, cdate.isSetPattern() ? cdate.getPattern() : null)
        .validate();

    // range check
    DvDate date = new DvDate(dvDateStr);

    if (cdate.isSetRange()) IntervalComparator.isWithinBoundaries(date, cdate.getRange());

    validateTimeZone(path, date, cdate);
  }

  private void validateTimeZone(String path, DvDate dvDate, CDATE cdate)
      throws IllegalArgumentException {
    // TODO: migrate to Temporal
    //        if (cdate.isSetTimezoneValidity() && cdate.getTimezoneValidity().equals(new
    // BigInteger("1001")) && dvDate.getValue().get(ChronoField.OFFSET_SECONDS) == null) {
    //            ValidationException.raise(path, "Time zone is mandatory", "DATE02");
    //        }
    //        if (cdate.isSetTimezoneValidity() && cdate.getTimezoneValidity().equals(new
    // BigInteger("1003")) && dvDate.getValue().getZone() != null) {

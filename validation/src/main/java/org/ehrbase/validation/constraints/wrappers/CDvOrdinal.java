      ...
      if (cdvordinal.sizeOfListArray() > 0) {
        for (DVORDINAL ordinal : cdvordinal.getListArray()) {
          if (ordinal.getValue() == dvOrdinal.getValue()) {
            // check symbol
            if (StringUtils.isNotEmpty(ordinal.getSymbol().getValue())
                && !(ordinal.getSymbol().getValue().equals(dvOrdinal.getSymbol().getValue())))
              continue;
            String codeString = dvOrdinal.getSymbol().getDefiningCode().getCodeString();
            String terminology =
                dvOrdinal.getSymbol().getDefiningCode().getTerminologyId().getValue();

            if (!(StringUtils.isNotEmpty(codeString)
                    && ordinal.getSymbol().getDefiningCode().getCodeString().equals(codeString))
                && (StringUtils.isNotEmpty(terminology)
                    && ordinal
                        .getSymbol()
                        .getDefiningCode()
                        .getTerminologyId()
                        .getValue()
                        .equals(terminology))) continue;
            break match_loop;
          }
        }

...
public class CReal extends CConstraint implements I_CTypeValidate {
  ...
  public void validate(String path, Object aValue, CPRIMITIVE cprimitive)
    ...
    if (aValue instanceof Double) aFloat = ((Double) aValue).floatValue();
    else if (aValue instanceof Float) aFloat = (Float) aValue;
    else if (aValue instanceof Integer) aFloat = ((Integer) aValue).floatValue();
    else
      ValidationException.raise(
          path, "Value could not be handled (is it numerical?)" + aValue, "FLOAT01");

    IntervalOfReal intervalOfReal = creal.getRange();
    if (intervalOfReal != null) IntervalComparator.isWithinBoundaries(aFloat, intervalOfReal);

    // check within value list if specified
    if (creal.sizeOfListArray() > 0 && !ArrayUtils.contains(creal.getListArray(), aFloat))
      ValidationException.raise(
          path, "Real value does not match any values in constraint:" + aFloat, "FLOAT02");
  }
}

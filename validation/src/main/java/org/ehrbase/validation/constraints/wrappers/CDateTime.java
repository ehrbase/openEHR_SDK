...
public class CDateTime extends CConstraint implements I_CTypeValidate {
  ...
  public void validate(String path, Object aValue, CPRIMITIVE cprimitive)
    ...
    if (!(aValue instanceof Temporal))
      ValidationException.raise(
          path, "INTERNAL: Time validation expects a java-date-time argument", "DATE_TIME_01");

    CDATETIME cdatetime = (CDATETIME) cprimitive;
    String dvDateStr = DateTimeFormatters.ISO_8601_DATE_TIME.format((Temporal) aValue);

    new DateTimeSyntax(path, dvDateStr, cdatetime.isSetPattern() ? cdatetime.getPattern() : null)
        .validate();
  ...
  private void validateTimeZone(String path, DvDateTime dvDateTime, CDATETIME cdatetime)
      throws IllegalArgumentException {
    //        if (cdatetime.isSetTimezoneValidity() && cdatetime.getTimezoneValidity().equals(new
    // BigInteger("1001")) && dvDateTime.getDateTime().getZone() == null) {
    //            ValidationException.raise(path, "Time zone is mandatory", "DATE02");
    //        }
    //        if (cdatetime.isSetTimezoneValidity() && cdatetime.getTimezoneValidity().equals(new
    // BigInteger("1003")) && dvDateTime.getDateTime().getZone() != null) {

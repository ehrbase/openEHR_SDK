 ...
 *
 * <p>Created by christian on 7/15/2016.
 */
public class CObjectConstraint extends ElementConstraint {
  ...
  public Boolean validate(DataValue value) throws IllegalArgumentException {
    ...
    //
    //            Class contraintTypeClass =
    // Class.forName(constraint.schemaType().getFullJavaName());
    //
    ...
    //                Method validator = constraintImpl.getDeclaredMethod("validate",
    // DataValue.class, contraintTypeClass);
    //                return (Boolean)validator.invoke(null, value, constraint);
    //            }
    //
    //        }
    //        else if (constraint instanceof CCOMPLEXOBJECT){
    //            String domainType = Utils.snakeToCamel(((CCOMPLEXOBJECT)
    // constraint).getRmTypeName());
    //            //get the corresponding class validation method
    //            if (VBeanUtil.isConstraintImplemented(domainType)){
    //                Class constraintImpl = VBeanUtil.findConstraintClass(domainType);
    //                //invoke the static validator using attributes
    //                Method validator = constraintImpl.getDeclaredMethod("validate",
    // DataValue.class, CATTRIBUTE[].class);
    //                return (Boolean)validator.invoke(null, value,
    // ((CCOMPLEXOBJECT)constraint).getAttributesArray());
    //            }
    //
    //        }
  ...
  //    private Class findImplementingConstraintClass(String rmTypeName) throws
  // ClassNotFoundException {
  ////        String constraintTypeName = ("C_"+rmTypeName).replaceAll("_", "");
  //        Class contraintTypeClass =
  // Class.forName(constraint.schemaType().getFullJavaImplName().split(".impl")[0]+"."+constraintTypeName);

...
public class CDvCodedText extends CConstraint implements I_CArchetypeConstraintValidate {
  ...
  private Logger logger = LoggerFactory.getLogger(CDvCodedText.class);
  ...
  public void validate(String path, Object aValue, ARCHETYPECONSTRAINT archetypeconstraint)
    ...
    if (!(archetypeconstraint instanceof CSINGLEATTRIBUTE))
      ValidationException.raise(
          path,
          "Constraint for DvCodedText is not applicable:" + archetypeconstraint,
          "CODED_TEXT_03");
    ...
    if (!(object instanceof CCODEPHRASE)) {
      ...
      ValidationException.raise(
          path, "Constraint child is not a code phrase constraint:" + object, "CODED_TEXT_02");
    }
    CCODEPHRASE ccodephrase = (CCODEPHRASE) object;

    // use code phrase validation checker
    new CArchetypeConstraint(localTerminologyLookup)
    ...
    if (ccodephrase.isSetTerminologyId()
        ...
        && (ccodephrase.getTerminologyId().getValue().equals("local")
            || ccodephrase
                .getTerminologyId()
                .getValue()
                .equals("openehr"))) // if null, the terminology is let free and we cannot check the
      // code

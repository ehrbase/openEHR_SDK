  ...
  public void validate(String path, Object aValue, ARCHETYPECONSTRAINT archetypeconstraint)
    ...
    if (quantity.getUnits() == null)
      ValidationException.raise(
          path, "No units specified for item:" + quantity + " at path:" + path, "DV_QUANTITY_02");
    ...
    match_value:
    {
      ...
      for (CQUANTITYITEM cquantityitem : constraint.getListArray()) {
        ...
        if (cquantityitem.isSetMagnitude() && quantity.getMagnitude() != null) {
          ...
          if (precision != null) {
            IntervalComparator.isWithinPrecision(
                precision.intValue(), cquantityitem.getPrecision());
          }
        }
      ...
      ValidationException.raise(
          path,
          "No matching units for:"
              + (StringUtils.isNotEmpty(quantity.getUnits()) ? quantity.getUnits() : "*undef*")
              + ", expected units:"

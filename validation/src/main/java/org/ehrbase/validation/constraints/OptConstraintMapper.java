...
public class OptConstraintMapper extends ConstraintMapper {
  ...
  void addToWatchList(String path, CSINGLEATTRIBUTE csingleattribute) {
    ...
    ConstraintOccurrences constraintOccurrences =
        new ConstraintOccurrences(csingleattribute.getExistence());
  ...
  void addToCardinalityList(String path, CMULTIPLEATTRIBUTE cmultipleattribute) {
    ...
    try {
      ...
      if (cardinalInterval.getLower() >= 1 || !cardinalInterval.getUpperUnbounded()) {
        CardinalityItem cardinalityItem =
            new CardinalityItem(
                new ConstraintOccurrences(cmultipleattribute.getExistence()),
                new ConstraintOccurrences(cmultipleattribute.getCardinality().getInterval()));

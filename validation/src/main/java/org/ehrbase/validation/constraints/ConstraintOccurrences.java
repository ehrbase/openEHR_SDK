...
/**
 * Container for IntervalOfInteger <v1:occurrences> <v1:lower_included>true</v1:lower_included>
 * <v1:upper_included>true</v1:upper_included> <v1:lower_unbounded>false</v1:lower_unbounded>
 * <v1:upper_unbounded>false</v1:upper_unbounded> <v1:lower>1</v1:lower> <v1:upper>1</v1:upper>
 * </v1:occurrences> Created by christian on 7/14/2016.
 */
public class ConstraintOccurrences implements Serializable {
  ...
  public ConstraintOccurrences(IntervalOfInteger intervalOfInteger) {
    ...
    if (intervalOfInteger.isSetLower()) lower = intervalOfInteger.getLower();
    else lower = Integer.MIN_VALUE;
    if (intervalOfInteger.isSetUpper()) upper = intervalOfInteger.getUpper();
  ...
  public MultiplicityInterval getExistence() {
    ...
    else if (lower == 0) return MultiplicityInterval.createOptional();
    else if (lower > 0) return MultiplicityInterval.createMandatory();

    throw new IllegalArgumentException(
        "Could not determine attribute existence from constraint (lower="
            + lower
            + ", upper="
            + upper
            + ")");

  ...
  public void validate(String path, Object aValue, CPRIMITIVE cprimitive)
    ...
    if (!(aValue instanceof Temporal))
      ValidationException.raise(
          path, "INTERNAL: Time validation expects a java-date-time argument", "TIME_01");

    CTIME ctime = (CTIME) cprimitive;
    String dvTimeStr = aValue.toString();

    // check pattern if any
    new DateTimeSyntax(path, dvTimeStr, ctime.isSetPattern() ? ctime.getPattern() : null)
        .validate();

    // range check
    DvTime time = new DvTime(dvTimeStr);

    if (ctime.isSetRange()) IntervalComparator.isWithinBoundaries(time, ctime.getRange());

    validateTimeZone(path, time, ctime);
  }

  private void validateTimeZone(String path, DvTime dvTime, CTIME ctime)
      throws IllegalArgumentException {
    //        if (ctime.isSetTimezoneValidity() && ctime.getTimezoneValidity().equals(new
    // BigInteger("1001")) && dvTime.getDateTime().getZone() == null) {
    //            ValidationException.raise(path, "Time zone is mandatory", "DATE02");
    //        }
    //        if (ctime.isSetTimezoneValidity() && ctime.getTimezoneValidity().equals(new
    // BigInteger("1003")) && dvTime.getDateTime().getZone() != null) {

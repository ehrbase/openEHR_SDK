...
public class NodeCounter {
  ...
  public void count(Object locatable) {
    ...
    if (locatable instanceof List)
      ...
      if (((Element) locatable).getNullFlavour() != null
          || ((Element) locatable).getValue() != null) count++;
    } else if (locatable instanceof ItemTree) count(((ItemTree) locatable).getItems());
    else if (locatable instanceof ItemList) count(((ItemList) locatable).getItems());
    else if (locatable instanceof ItemSingle)
      count += ((ItemSingle) locatable).getItem() != null ? 1 : 0;
    else if (locatable instanceof Section) count(((Section) locatable).getItems());
      ...
      throw new IllegalArgumentException(
          "Unhandled data type:"
              + locatable
              + ", class:"
              + locatable.getClass().getCanonicalName());
  }

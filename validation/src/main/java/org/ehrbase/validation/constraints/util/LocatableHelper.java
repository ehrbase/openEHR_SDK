...
public class LocatableHelper {
  ...
  private Map<String, Integer> arrayItemPathMap =
      new HashMap<>(); // contains the list of array insertion paths for a template
    ...
    for (Object sibling : siblings) {
      ...
      if (sibling instanceof Locatable) {
        ...
        if (!nodeId.contains("openEHR")) {
          ...
          if (current > last) lastNodeId = "at" + String.format("%04d", current);
        } else {
  ...
  public static String indentifyName(String path) {
    ...
    if (index < 0) return null; // path such as /ism_transition f.ex.
    ...
    if (expression.contains(" AND ")
        ...
        || expression.contains(COMMA_TOKEN)) {
      ...
      if (expression.contains(" AND ")) {
        ...
        index = expression.indexOf(" and ");
      } else index = expression.indexOf(COMMA_TOKEN);
  ...
  public static String siblingPath(String unresolvedPath) {
    ...
    if (last.contains(AND_NAME_VALUE_TOKEN) || last.contains(COMMA_TOKEN)) {
      last =
          last.contains(AND_NAME_VALUE_TOKEN)
              ? last.substring(0, last.indexOf(AND_NAME_VALUE_TOKEN)) + CLOSE_BRACKET
              : last.substring(0, last.indexOf(COMMA_TOKEN)) + CLOSE_BRACKET;
    }
  ...
  public static Locatable siblingAtPath(Locatable locatable, String unresolvedPath) {
    ...
    if (sibling != null) return (Locatable) sibling;

    return null;
  }

  /**
   * return the first parent matching an unresolved path by identify the first item which path
   * matches fully a partial path expression
   *
   * @param locatable
   * @param unresolvedPath
   * @return
  ...
  public static NodeItem backtrackItemAtPath(Locatable locatable, String unresolvedPath) {
    ...
    while (parentAtPath == null && parentPath != null && parentPath.length() > 0) {
      ...
      if (parentPath == null || parentPath.length() <= 0) break;
      childAtPath = parentAtPath;
      parentAtPath = locatable.itemAtPath(parentPath);
    }

    return new NodeItem(
        (Locatable) parentAtPath, lastPath, FORWARD_SLASH + identifyAttribute(lastPath));
  }
  ...
  public static String getLocatableParentPath(Locatable locatable, String path) {
    ...
    for (int i = segments.size() - 1; i >= 0; i--) {
      String parentPath = FORWARD_SLASH + String.join(FORWARD_SLASH, segments.subList(0, i));
      if (locatable.itemAtPath(parentPath) instanceof Locatable) return parentPath;
    }
    return null;
  }
      ...
      if (lastNodeid.contains(AND_NAME_VALUE_TOKEN) || lastNodeid.contains(COMMA_TOKEN)) {
        if (lastNodeid.contains(INDEX_PREFIX_TOKEN)) {
          ...
          for (Locatable locatable : itemList) {
            if (locatable.getName().getValue().equals(nameValueToken)) return locatable;
          }
        }
      }
    }
  ...
  public static String simplifyPath(String path) {
    ...
    for (int i = 0; i < segments.size(); i++) {
      ...
      if (segment.contains(AND_NAME_VALUE_TOKEN) || segment.contains(COMMA_TOKEN)) {
        String trimmedNodeId =
            segment.contains(AND_NAME_VALUE_TOKEN)
                ? segment.split(AND_NAME_VALUE_TOKEN)[0]
                : segment.split(COMMA_TOKEN)[0];
        tentativePath.append(trimmedNodeId.trim());
        tentativePath.append(CLOSE_BRACKET);
      } else tentativePath.append(segment);
      if (i < segments.size() - 1) tentativePath.append(FORWARD_SLASH);
    }
  ...
  public static boolean hasDefinedOccurence(String path) {
    ...
    for (int i = 0; i < segments.size(); i++) {
      ...
      if (segment.contains(AND_NAME_VALUE_TOKEN) || segment.contains(COMMA_TOKEN)) {
        ...
        String namePart =
            segment.contains(AND_NAME_VALUE_TOKEN)
                ? segment.split(AND_NAME_VALUE_TOKEN)[1]
                : segment.split(COMMA_TOKEN)[1];
        if (namePart.contains(INDEX_PREFIX_TOKEN)) return true;
      }
    }
    ...
    if (nodeid.contains(AND_NAME_VALUE_TOKEN) || nodeid.contains(COMMA_TOKEN))
      return (nodeid
                  .substring(
                      0,
                      nodeid.indexOf(
                          nodeid.contains(AND_NAME_VALUE_TOKEN)
                              ? AND_NAME_VALUE_TOKEN
                              : COMMA_TOKEN))
                  .trim()
              + CLOSE_BRACKET)
          .trim();
  ...
  public void addItemPath(String itemPath) {
    if (!arrayItemPathMap.containsKey(itemPath)) arrayItemPathMap.put(itemPath, 1);
    else arrayItemPathMap.put(itemPath, arrayItemPathMap.get(itemPath) + 1);
  }

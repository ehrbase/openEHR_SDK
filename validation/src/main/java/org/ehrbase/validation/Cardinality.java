...
public class Cardinality {
   ...
   */
  public void check(
      Locatable structure, String path, ConstraintMapper.CardinalityItem cardinalityItem) {
      ...
      if (childOccurrence == 0 && !(cardinalityItem.getExistence().isOptional())) {
        ...
        if (!isTransitivelyOptional(path))
          ValidationException.raise(
              path,
              "Cardinality not matched, expected:"
                  + IntervalComparator.toString(cardinalityItem.getCardinality().asInterval())
                  + ", actual:"
                  + childOccurrence,
              "CAR01");
  ...
  /**
   * check if the parent container is *really* optional, that is its lower cardinality is 0 and it
   * is not used... NB. There are many discussions about this cardinality problem, presumably fixed
   * with ADL2 (?)
   *
   * @param path
   * @return
  ...
  /**
   * Validate the cardinality of an item, that is whether the item is within the cardinality
   * boundaries
   *
  ...
  public String validate() {
    ...
    for (Map.Entry<String, ConstraintMapper.CardinalityItem> entry :
        constraintMapper.getCardinalityList().entrySet()) {
      valcount++;
            ...
            exceptions.append(
                new Message()
                    .encode(
                        entry.getKey(),
                        "Cardinality not matched, expected:"
                            + IntervalComparator.toString(
                                entry.getValue().getCardinality().asInterval())
                            + ", actual:"
                            + childOccurrence,
                        "CAR01"));

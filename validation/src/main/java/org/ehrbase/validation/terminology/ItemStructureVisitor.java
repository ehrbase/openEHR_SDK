...
/**
 * Visitor of an item structure to perform specific validations: - CodePhrase depending on
 * terminology - units
 */
public class ItemStructureVisitor implements I_ItemStructureVisitor {
  ...
  private String itemStructureLanguage =
      "en"; // if a composition, the language can be found in the structure
  ...
  private void traverse(ContentItem item) throws IllegalArgumentException, InternalError {
    ...
    log.debug(
        "traverse element of class:" + item.getClass() + ", nodeid:" + item.getArchetypeNodeId());
  ...
  private void traverse(Activity activity) throws IllegalArgumentException, InternalError {
    ...
    traverse(
        activity
            .getDescription()); // don't add a /data in path for description (don't ask me why...)
  }
  ...
  private void validateElement(Element element) throws IllegalArgumentException, InternalError {
    ...
    if (element.getNullFlavour() != null
        && itemValidator.isValidatedRmObjectType(element.getNullFlavour())) {
      itemValidator.validate(
          localizedTerminologies.locale(itemStructureLanguage),
          codesetMapping,
          "null_flavour",

...
public class TerminologyCheck implements I_TerminologyCheck {
  ...
  public static void validate(
    ...
    if (codePhrase.getTerminologyId().getValue().equals("local")) return;

    // get the actual attribute
    if (!codesetMapping.isLocalizedAttribute(
        codePhrase.getTerminologyId().getValue(), context, language))
      language = "en"; // default to English for the rest of the validation

    String attribute =
        codesetMapping.actualAttributeId(
            codePhrase.getTerminologyId().getValue(), context, language);
    ContainerType containerType =
        codesetMapping.containerType(codePhrase.getTerminologyId().getValue(), context);
    ...
    switch (containerType) {
      ...
      case CODESET: // a codestring defined in a codeset
        valid =
            terminologyInterface
                .codeSet(codePhrase.getTerminologyId().getValue())
                .hasCode(codePhrase);
        if (!valid) {
          throw new IllegalArgumentException(
              "supplied code string ["
                  + codePhrase.getCodeString()
                  + "] is not found in codeset:"
                  + attribute);
        }
        break;
  ...
  public static void validate(
      ...
      throws IllegalArgumentException {
    validate(
        terminologyInterface, codesetMapping, context, dvCodedText.getDefiningCode(), language);
